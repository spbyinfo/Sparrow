...
const getEnvVar = (key: string, defaultValue: string = ''): string => {
  try { if (typeof import.meta !== 'undefined' && import.meta.env) return import.meta.env[key] || defaultValue } catch (e) {}
  return defaultValue
}

const getCurrentEnvironment = (): Environment => {
  const env = getEnvVar('VITE_ENV', 'staging')
  if (env === 'production' || env === 'pre-production') return env as Environment
  return 'staging'
}

const configs: Record<Environment, Partial<EnvironmentConfig>> = {
  staging: { enableAnalytics: false, enableDebugMode: true, showEnvironmentBadge: true },
  'pre-production': { enableAnalytics: true, enableDebugMode: true, showEnvironmentBadge: true },
  production: { enableAnalytics: true, enableDebugMode: false, showEnvironmentBadge: false },
}
...
export const devLog = (...args: any[]) => { if (environment.enableDebugMode) console.log('[Sparrow Debug]', ...args) }

export const getEnvironmentBadgeConfig = () => {
  if (!environment.showEnvironmentBadge) return null
  const badges = { staging: { label: 'STAGING', color: '#ffd700', bgColor: '#ffd70020' }, 'pre-production': { label: 'PRE-PROD', color: '#ff9933', bgColor: '#ff993320' } }
